...
Documentation can be accessed through the console by two different ways. Printing using the __doc__ attribute or using the help() function. The help function tends to be a little more verbose and simpler.
...
x *= 2             # Multiply and store the result back in x
...
The above is exactly the same as the previous example. The one thing to keep in mind is that the number of variables created, has to match the number of elements within the list. Each variable assigned represents each index value. If an element or variable was missing, Python will throw an error.
...
The variable with the asterisk does not have to be last, in the list, you can move it around. The key thing here is that the asterisk is placed before the variable. Note that unpacking tuples works in exactly the same way.
...
We can perform a number of operations on a set. Keep in mind that set elements have to be immutable, which means you can’t have lists or dictionaries within a set.

{% highlight python linenos %}
first_numbers = {1,2,3,4}
second_numbers = {3, 4, 10, 20, 30, 40}
 
# Adding an item to a set. Adding items which already exists will have no effect.
second_numbers.add(7)
 
# Remove an item from a set. Exception will be raised if the item is not in the set.
...
# Return the values that exists in either sets but not both.
print (first ^ second)
{% endhighlight %}
...
ANother use case here is say, you are working on a translator and you need to to convert the months of the year from one language to another. With dictionaries, you can have each of the month represented as separate individual keys and then the value can become whatever language that key needs to translate into.
...
{% endhighlight %}
If the key values are just basically, simple strings, those key-value pairs can be specified as keyword arguments. Therefore, another way of writing the dictionary above, could be done like so:
...
Python has several functions built-in for manipulating those dictionaries, one of which is to query if a key exists inside of a dict. Using the get() function, we can check to see if a specific key exist, if it doesn’t Python will return a ‘None’ instead of raising an exception.
...
{% endhighlight %}
Because of this, dictionaries become an extremely useful and powerful tool when working with different sorts of data. As such, there are a number of buil-in functions to help us manage and work with these dictionaries.
...
# Removes a key from a dictionary and returns it's value.
...
There’s a lot going on with this string. Several concatenations, a type conversion and a lot of spaces in very specific place. This makes things hard to keep track of and difficult to read.

String formatting allows us to simplify a lof of this clutter, making it easier to manage and thus format our strings. There are several methods of formatting strings. Python 2 popularly used the .format() function. The formula goes something like below:
...
{% endhighlight %}
We can also store our variables within a dictionary and call on each of the dictionary keys within out parenthesis.
...
But notice how we’re repeating our dictionaty call in the format function 3 times? Here, we’re essentiallt calling out dictionary 3 times which we don’t need to, since we already know out dictionary keys
...
Python resolves this. The same would be true if the quotes were reversed. This works but it breaks any established convention you may have and doesn’t really solve the problem. Say you had a sentence that is encased in single quotes ' and had words containing '. You’ll likely still run into issues.

Instead it is better to use escape sequences so that you can maintain consistency. They work like this. Your entire string gets enclosed within quotation marks, whether it’s single 'or double ". If you want to add other quotation marks within your string, you simply need to insert a backslash \, which is the symbol for escaping a sequnce, before your quotation marks. This becomes extremely important, expecially when dealing with paths. Here are some examples of using escape sequences.
...
#  Using escape sequence to add single backslash to a string.
...
#  Using escape sequence to add double backslash to a string.
print('c:\\\\some_folder\\\\some_sub_folder\\\\file.ext')
